...
- The model runs on **Ollama, local or cloud** (cloud resources are fine). The runner currently points at the local Ollama daemon (`http://localhost:11434`), which serves both local models and `:cloud` models; model `deepseek-v4.1-flash:cloud` (must match `ollama list` exactly). Auth uses `OLLAMA_API_KEY` from `.zshrc`; the runner extracts it because launchd does not source the shell.
- FAL is available for image generation if a future report ever needs it (the key resolves through `FAL_KEY` from the login keychain `huffmanwrites-fal`, with `~/.secrets` as fallback); the weekly report does not use hero images.
...
3. Web-search the latest on every competitive race and scan the safe ones: Cook Political Report, Sabato's Crystal Ball, RealClearPolitics, FiveThirtyEight, prediction markets, FEC filings, and local/state news. Corroborate key claims with at least two sources. Search results and secondary coverage *quote* the forecasters, so the paywalled/blocked pages can still be sourced that way — but the ratings table itself comes from the script.
   ...
   - **Market prices come from the exchange's own record, never from an aggregator's summary of it.** This rule exists because the same defect shipped twice in one week (September 13 and September 20, 2026): a price attributed to Kalshi that Kalshi's own desk did not report. Both times the number came from a secondary outlet — a headline like "Democrats 60% Senate" or "Democrats Flip Senate Odds to 51%" — that was quoting the market rather than being it. The market's operator is the authority on its own price, and it publishes its own record, so there is no excuse for the substitution.
     - **Kalshi publishes its own numbers at `news.kalshi.com`.** `kalshi.com` itself returns **429** to automated fetch, and its trading API (`api.elections.kalshi.com/trade-api/v2`) answers 200 but does not expose a Senate-control series under the obvious tickers — so the news desk is the route. Its `sitemap.xml` lists every post with a `<lastmod>` and is the way to find them when search is unavailable: fetch `https://news.kalshi.com/sitemap.xml`, filter slugs for `senate` / `midterm` / `democrat`, and read the posts. Cite the post and quote its wording verbatim.
     - **A price is a point-in-time reading, not a fact that stays true.** Say when it was taken ("a record 57 percent on September 17"), never restate an older price as current, and never present an aggregator's later reading as the exchange's own. If the only number you can find is an aggregator's, either omit the price or attribute it plainly ("Picks & Parlays reported the market near 60 percent") — do not launder it into "Kalshi prices 60 percent."
     - **Do not assert a market *history* claim without the operator's record.** "The first Democratic price since April" is a claim about a series of past prices; a current price does not establish it, and a single aggregator cannot. Kalshi's news desk documents its own history (e.g. 51 percent on March 14, 52 percent on April 20), and only that record can carry a "first since…" claim.
4. Track: rating changes, polling movement, fundraising, endorsements, candidate news, debates, and races entering or leaving the competitive tier.
