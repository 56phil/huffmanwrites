...
- Hero images: generate a fresh pair per article via the **fal.ai** images API (FLUX.1 dev, `FAL_KEY` from `.zshrc`, endpoint `https://queue.fal.run/fal-ai/flux/dev`, `Authorization: Key $FAL_KEY`). Policy (Philip, 2026-09-06): no Anthropic or OpenAI resources; the only AI API keys available are FAL and Ollama. If a fal.ai call fails with an auth/billing error, do not debug or retry the account — note it in the output and stop.
