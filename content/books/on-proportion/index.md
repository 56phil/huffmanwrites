---
title: "On Proportion"
subtitle: "Growing up with the Cold War"
description: |
  Now available.

  On Proportion examines the lost discipline of matching response to scale: knowing when a situation demands attention and when it demands inattention, when intervention is warranted and when silence is the better instrument. It is a discipline Philip Huffman learned growing up in the shadow of the Cold War, an era that specialized in treating every tremor as the end of the world.

  The book treats proportion not as moderation but as judgment — the capacity to sense the true size of an event and to resist the distortions of urgency, outrage, and habit. Against the backdrop of a superpower's machinery of overreaction, it traces how a generation was trained to respond to everything, and how that training outlived the era that produced it.

  Written for anyone who has felt the pull to answer everything and the cost of doing so, On Proportion offers a way to tell the urgent from the important — and to choose silence when silence is the better instrument.
weight: 8
image: "img/books/proportion.jpg"
image_desktop: "img/articles/op-16x9.webp"
image_mobile: "img/articles/op-4x5.webp"
image_alt: "Intricate gold-filigree vines and flowers climbing and blooming across a massive, oppressive Cold War-era brutalist concrete wall."
hero_caption: "Gold filigree vines climb a brutalist wall — the organic against the rigid."
image_caption: "The organic growth of the spirit against the cold architecture of an era."
lastmod: 2026-05-19
---
## Formation

## Application

## Cost

## Reckoning

